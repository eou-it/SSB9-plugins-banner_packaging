...
//---------------------------- private methods ---------------------------------
    ...
    private void updateWAR() throws ActionRunnerException {
       ...
       // fs.createExclude().setName( "**WEB-INF/web.xml" )
        war.addFileset( fs )
        boolean requireWebxml = false
        war.setNeedxmlfile(requireWebxml)
        runTask( war )
    }
...
//                        println "Going to remove CAS filter-mapping at index $index:  ${elem.'filter-name'.toString()}"
	...
	private void updateI18N( File stagingDir ) {	
		updateStaging( stagingDir, "WEB-INF/classes", FileStructure.I18N_DIR )
		updateStaging( stagingDir, "WEB-INF/classes", FileStructure.INSTANCE_I18N_DIR )
	}
